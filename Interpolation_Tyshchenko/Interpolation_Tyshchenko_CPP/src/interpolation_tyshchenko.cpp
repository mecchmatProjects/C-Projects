...
DType Project::power(DType x, IType n){
    DType res = 1;
    for (int i = 0; i < n; ++i) {
        res *= x;
    }
...
DVector Project::SolveLinSystem(DType** matrix, DType *vector, IType n){
...
void Project::Jacobi (IType n, DType** matrix, DType* f, DType* result)
{
    const DType eps = 0.1;
...
IType Project::inputTextFile(char *file) {
...
void Project::output(DVector *result) {
    printf("y=");
...
int Project::outputBinaryFile(char *file, DVector *result){
...
int Project::outputTextFile(char *file, DVector *result){
