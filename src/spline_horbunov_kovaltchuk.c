...
void free_polynomial(Polynomial p) { free(p.coefficients); }

// UTILITY FUNCTIONS FOR STRUCTURE NAMED DEFECT
void free_defect(Defect d) { free(d.net); }
/*
...
Polynomial integral_of_polynomial(Polynomial x, float c) {
    ...
    for (unsigned i = 1; i < result.n; i++) { result.coefficients[i - 1] = x.coefficients[i] * (i); }
...
Spline add_spline(Spline s_0, Spline s_1) {
    ...
    result.defect_struct = s_0.defect_struct;
    result.n = s_0.n;
...
Spline derivative_of_spline(Spline s) {
    ...
    result.defect_struct = s.defect_struct;
    result.n = s.n - 1;
